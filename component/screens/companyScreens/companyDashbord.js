...
function companyDashbord({route, navigation}) {
    const {email,_name, address, employes, number} = route.params
    ...
    function findStudent() {
     ...
     .once('value', function(data){
      data.forEach((childSnapshot) => {
        ...
        navigation.navigate("CompanyFindStudent",{
        name: name,
        email:email,
    ...
    return(
        ...
        <Container style={{backgroundColor:'#fff', flex:1}}>
          ...
          <Text style={styles.text} >Name: {_name}</Text>
            ...
            <Text style={styles.text} >employes:{employes}</Text>
       ...
       <Button active onPress={()=>findStudent()}>
         ...
         <Button active onPress={()=>navigation.push("CompanyPostJob")}>
